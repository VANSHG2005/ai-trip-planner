      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity:0, y:6, scale:0.96 }} animate={{ opacity:1, y:0, scale:1 }}
            ...
            className="absolute right-0 top-full mt-2 w-52 z-50 card-premium overflow-hidden" style={{ maxHeight:'280px', overflowY:'auto' }}>
            <div className="p-1.5">
              ...
              {currencies.map(c => (
                <button key={c.code} onClick={() => { setCurrency(c); setOpen(false) }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm transition-colors ${currency.code===c.code ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-muted text-foreground'}`}>
